...
with tf.name_scope("weights_and_biases"):
    ...
    hidden1_weights = tf.Variable(initial_value=tf.random_uniform([n_inputs, n_hidden_1], -0.1, 0.1), dtype=tf.float32, name="hidden1_weights")
    hidden1_biases = tf.Variable(initial_value=tf.random_uniform([n_hidden_1], -0.1, 0.1), dtype=tf.float32, name="hidden1_biases")
    
    # 第二层权值矩阵 300x100 ，偏置向量 100 。
    hidden2_weights = tf.Variable(initial_value=tf.random_uniform([n_hidden_1, n_hidden_2], -0.1, 0.1), dtype=tf.float32, name="hidden2_weights")
    hidden2_biases = tf.Variable(initial_value=tf.random_uniform([n_hidden_2], -0.1, 0.1), dtype=tf.float32, name="hidden2_biases")
    
    # 输出层权值矩阵 100x10 ，偏置向量 10 。
    output_weights = tf.Variable(initial_value=tf.random_uniform([n_hidden_2, n_outputs], -0.1, 0.1), dtype=tf.float32, name="output_weights")
    output_biases = tf.Variable(initial_value=tf.random_uniform([n_outputs], -0.1, 0.1), dtype=tf.float32, name="output_biases")

# 定义网络的计算图。
with tf.name_scope("network_flow"):
    
    # 第一层的计算。
    output_hidden1 = activation_fn_hidden1(tf.nn.bias_add(tf.matmul(X, hidden1_weights), hidden1_biases), name="hidden1")
    
    # 二层的计算。
    output_hidden2 = activation_fn_hidden2(tf.nn.bias_add(tf.matmul(output_hidden1, hidden2_weights), hidden2_biases), name="hidden2")
